...
class GdThumb extends ThumbBase {
    ...
    public function watermark($src, $mark, $dest, $top, $left) {
        // Load the stamp and the photo to apply the watermark to

        if (strpos($src, '.jpg') || strpos($src, '.jpeg') || strpos($src, '.JPEG') || strpos($src, '.JPG')) {
            $im = imagecreatefromjpeg($dest . $src);
        } else {
            if (strpos($src, '.png') || strpos($src, '.PNG')) {
                $im = @imagecreatefrompng($dest . $src);
            } else {
                $im = imagecreatefromgif($dest. $src);
            }
        }
        $stamp = imagecreatefrompng($dest.$mark);
        ...
        if (strpos($src, '.jpg') || strpos($src, '.jpeg') || strpos($src, '.JPEG') || strpos($src, '.JPG')) {
            imagejpeg($im, $dest .'watermark/'. $src, 100);
        } else {
            if (strpos($src, '.png') || strpos($src, '.PNG')) {
                $this->save_imagepng($im, $dest .'watermark/'. $src);
            } else {
                imagegif($im, $dest . 'watermark/'.$src, 100);
            }
        }
    ...
    public function save_imagepng($img, $fname) {
